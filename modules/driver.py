...
class ImageOp():
	...
	def norm_mask(self, area_th, scale):
		...
		self.mask = process.morphology(self.mask, 3, 3)

		# 輪郭抽出
		# contours, self.mask = process.get_norm_contours(morpho_mask, scale, 15)
		contours, self.mask = process.get_norm_contours(self.mask, scale, 3)

		# 面積が閾値未満の領域を除去
		self.mask = process.remove_small_area(contours, area_th, scale, self.mask)
	...
	def extract_sediment(self):
		...
		"""
		# 土砂マスクを用いて土砂領域以外を除去
		sed = process.extract_sediment(self.ortho, self.mask)
	...
	def calc_sedimentation(self):
		...
		# 堆積領域と侵食領域で二値化
		dsm_bin = process.bin_2area(self.dsm_sub)
	...
	def calc_movement(self):
		...
		## 傾斜方向が上から下の領域の組を全て抽出
		area_list2 = process.extract_direction(self.degree, self.dem, self.dsm_uav)

		## 侵食と堆積の領域の組を全て抽出
		area_list3 = process.extract_sub(self.dsm_sub)
		...
		# 土砂移動図の作成
		process.make_map(area_list, self.dsm_uav, self.path_list[5])
